classdef HumerusSSMApp < handle
    ...
    properties (Constant)
        AppPathBase = {'/HumerusSSM'};
        AppClass = 'HumerusSSMApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            HumerusSSMApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'Main_Humerus_SSM')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong Main_Humerus_SSM
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'Main_Humerus_SSM').getString, ...
            ...
            if nargout(@Main_Humerus_SSM) == 0
                eval('Main_Humerus_SSM');
            else
                obj.AppHandle = eval('Main_Humerus_SSM');
            end
